...
//	}
	...
	public ArrayList<StudentDTO> studentadd(ArrayList<StudentDTO> ar) {
		...
		while(check) {
			...
			for(int i = 0  ; i < ar.size(); i++) {
				if(ar.get(i).getNum()==(studentDTO.getNum())) {
					...
					j++;
					break;
				}else if(ar.get(i).getNum()!=(studentDTO.getNum()) && i==ar.size()-1) {
					System.out.println(i);
					check=false;
				}else if(j==2) {
					System.out.println("확인하고 오셈");
		...
		System.out.println("수학점수를 입력하시오");
		String math = sc.next();
		String temp = name+"-"+Integer.toString(studentDTO.getNum())+"-"+kuk+"-"+eng+"-"+math;
...
//	}
	...
	public boolean studentdelect(ArrayList<StudentDTO> ar) {
		//학생 이름을 입력받아
		// 찾아서 해당 학생 제거
		System.out.println("삭제할 학생의 이름을 입력하시오");
		String inputName = sc.next();
		...
		for(StudentDTO studentDTO:ar) {
			...
			if(studentDTO.getName().equals(inputName)==true) {
